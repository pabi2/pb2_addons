...
class PurchaseOrderLine(models.Model):
    ...
    def default_get(self, fields):
        ...
        order_line = self.browse(active_id)._context.get('order_line', [])
        if order_line != []:
            order_id = max(order_line)[1]
            last_rec = self.search([
                ('id', '=', order_id)], order='id desc', limit=1)
            if last_rec:
    ...
    def onchange_product_id(
        ...
        if order_line != []:
            ...
            if last_rec.product_id.name == res['value']['name'] or \
                    last_rec.name == res['value']['name']:
                res['value']['product_id'] = last_rec.product_id.id
                res['value']['name'] = last_rec.name
                res['value']['date_planned'] = date_now
                res['value']['activity_group_id'] =\
                    last_rec.activity_group_id.id
                ...
                if not ('product_qty' in res['value']):
