...
class BudgetReportByCostCentreXLS(report_xls):
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        actual_formula_list = []
        for row_pos_total in row_pos_total_list:
            budget_control_formula_list.append(
                rowcol_to_cell(row_pos_total - 1, 3))
            budget_release_formula_list.append(
                rowcol_to_cell(row_pos_total - 1, 4))
            expense_commitment_formula_list.append(
                rowcol_to_cell(row_pos_total - 1, 5))
            pr_commitment_formula_list.append(
                rowcol_to_cell(row_pos_total - 1, 6))
            po_commitment_formula_list.append(
                rowcol_to_cell(row_pos_total - 1, 7))
            actual_formula_list.append(rowcol_to_cell(row_pos_total - 1, 9))
