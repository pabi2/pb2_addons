...
class account_voucher(osv.osv):
    ...
    def recompute_partner_voucher_lines(self, cr, uid, ids,
                                        partner_id, journal_id,
                                        price, currency_id, ttype,
                                        date, context=None):
        def _remove_noise_in_o2m():
            """if the line is partially reconciled,
                then we must pay attention to display it only once and
                in the good o2m.
        ...
        ids.reverse()
        account_move_lines =\
            move_line_pool.browse(cr, uid, ids, context=context)

        # compute the total debit/credit and
        # look for a matching open amount or invoice
        ...
        for line in account_move_lines:
            ...
            rs = {
                ...
                min(abs(price), amount_unreconciled) or 0.0,
                'date_original': line.date,
                'date_due': line.date_maturity,
                'amount_unreconciled': amount_unreconciled,
                'currency_id': line_currency_id,
            }
            # in case a corresponding move_line hasn't been found,
            # we now try to assign the voucher amount
            # on existing invoices: we split voucher amount by
            # most old first, but only for lines in the same currency
            ...
            default['value']['writeoff_amount'] =\
                self._compute_writeoff_amount(cr, uid,
    ...
    def recompute_voucher_lines(self, cr, uid, ids, partner_id, journal_id,
        ...
        if context.get('mode', False) == 'partner':
            return self.recompute_partner_voucher_lines(cr, uid, ids,
                                                        partner_id,
                                                        journal_id,
                                                        price, currency_id,
                                                        ttype, date, context)
        else:
            return super(account_voucher, self).\
