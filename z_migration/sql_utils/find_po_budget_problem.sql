...
from purchase_order po where state not in ('draft', 'cancel') and currency_id = 140 and order_type = 'purchase_order'
-- Not COD case where its invoice is paid without number
and po.id not in (select purchase_id from purchase_invoice_rel where invoice_id in (select id from account_invoice where state != 'draft' and number is null))
