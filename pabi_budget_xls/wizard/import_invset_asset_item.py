    ...
    def import_budget(self):
        ...
        for plan in Plans:
            ...
            for row in range(line_row, max_row):
                ...
                asset_category_id =\
                    self.env['res.invest.asset.category'].search(
                        [('name', '=', tools.ustr(asset_category))])
                if asset_category_id:
                    line_vals.update(
                        {'invest_asset_categ_id': asset_category_id.id})
                ...
                reason_purchase_text =\
                    Asset_Sheet.cell(row=row, column=16).value
                if reason_purchase_text == '=FALSE()':
                    reason_purchase_text = ''
                line_vals.update(
                    {'reason_purchase_text': reason_purchase_text})

                planned_utilization =\
                    Asset_Sheet.cell(row=row, column=18).value
                ...
                specification_summary =\
                    Asset_Sheet.cell(row=row, column=20).value
                ...
                pr_commitment =\
                    Asset_Sheet.cell(row=row, column=23).value or 0.0
                line_vals.update({'pr_commitment': pr_commitment})

                po_commitment =\
                    Asset_Sheet.cell(row=row, column=24).value or 0.0
                line_vals.update({'po_commitment': po_commitment})

                exp_commitment =\
                    Asset_Sheet.cell(row=row, column=25).value or 0.0
                line_vals.update({'exp_commitment': exp_commitment})

                total_commitment =\
                    pr_commitment + po_commitment + exp_commitment
                line_vals.update({'total_commitment': total_commitment})

                actual_amount =\
                    Asset_Sheet.cell(row=row, column=26).value or 0.0
                line_vals.update({'actual_amount': actual_amount})

                budget_residual =\
                    Asset_Sheet.cell(row=row, column=28).value or 0.0
