...
class AccountTaxReportWizard(models.TransientModel):
    ...
    def run_report(self):
        ...
        data['parameters']['taxbranch_id'] = taxbranch.id
        # Display
        data['parameters']['branch_name'] = taxbranch.name
        data['parameters']['branch_vat'] = taxbranch.taxid
        data['parameters']['branch_taxbranch'] = taxbranch.code
        # Advance sequence
        if self.tax_id.type_tax_use == 'purchase':
            data['parameters']['advance_sequence'] = True
        else:
