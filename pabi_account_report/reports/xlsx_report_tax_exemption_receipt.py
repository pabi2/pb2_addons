...
class XLSXReportOutputTax(models.TransientModel):
    ...
    def _compute_results(self):
        ...
        if self.calendar_period_id:
            dom += [('invoice_id.period_id', '=', self.calendar_period_id.id)]
        if self.taxbranch_id:
            dom += [('invoice_id.taxbranch_id', '=', self.taxbranch_id.id)]
    ...
    def _get_sql_view(self):
        sql_view = """
            SELECT id, id AS invoice_id,
                ...
                number, NULL AS document_origin, validate_user_id
            ...
            GROUP BY invf.id, invf_line.date, invf.preprint_number, invf_line.partner_id
        """
        return sql_view
