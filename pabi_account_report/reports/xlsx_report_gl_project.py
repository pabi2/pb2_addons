    ...
    def _compute_tax(self):
        for move in self:
            if move.doctype == 'interface_account': 
                ...
                tax = []
                for invoice_line in move.move_id.invoice_ids.tax_line:
                    if invoice_line.tax_code_id:
                        tax.append(invoice_line.tax_code_id.tax_ids.description)
                move.tax_code = ", ".join(tax) 
#                 try:
#                     move.tax_code = move.move_id.invoice_ids.tax_line.tax_code_id.tax_ids.description  
#                 except Exception:
#                     tax = []
#                     for invoice_line in move.move_id.invoice_ids.tax_line:
#                         if invoice_line.tax_code_id:
