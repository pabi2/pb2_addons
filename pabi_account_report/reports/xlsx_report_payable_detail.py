...
class XLSXReportPayableDetail(models.TransientModel):
    ...
    def _compute_results(self):
        ...
        dom = [('account_invoice_move_id.state', '=', 'posted'),
               ('account_voucher_move_id.state', '=', 'posted'),
               ('voucher_id.state', '=', 'posted'),
               ('export_id.state', '=', 'done'),
               ('account_invoice_move_line_id.account_id', 'in',
                self.account_ids.ids)]
        if self.partner_ids:
            dom += [('account_invoice_move_line_id.partner_id', 'in',
                     self.partner_ids.ids)]
        if self.start_fiscalyear_id:
            dom += [('account_invoice_move_line_id.period_id.fiscalyear_id'
                     '.date_start', '>=', self.start_fiscalyear_id.date_start)]
        if self.end_fiscalyear_id:
            dom += [('account_invoice_move_line_id.period_id.fiscalyear_id'
                     '.date_start', '<=', self.end_fiscalyear_id.date_start)]
        if self.start_period_id:
            dom += [('account_invoice_move_line_id.period_id.date_start', '>=',
                     self.start_period_id.date_start)]
        if self.end_period_id:
            dom += [('account_invoice_move_line_id.period_id.date_start', '<=',
                     self.end_period_id.date_start)]
        if self.start_doc_posting_date:
            dom += [('account_invoice_move_id.date', '>=',
                     self.start_doc_posting_date)]
        if self.end_doc_posting_date:
            dom += [('account_invoice_move_id.date', '<=',
                     self.end_doc_posting_date)]
        if self.start_doc_date:
            dom += [('account_invoice_move_id.date_document', '>=',
                     self.start_doc_date)]
        if self.end_doc_date:
            dom += [('account_invoice_move_id.date_document', '<=',
                     self.end_doc_date)]
        if self.move_ids:
            dom += [('account_invoice_move_id', 'in', self.move_ids.ids)]
