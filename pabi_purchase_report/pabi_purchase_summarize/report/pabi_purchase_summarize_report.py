...
class PabiPurchaseSummarizeReport(models.Model):
    ...
    def init(self, cr):
        ...
        cr.execute("""CREATE or REPLACE VIEW %s as (
                ...
                CONCAT((SELECT pt.name FROM purchase_type pt WHERE pt.id = pd.purchase_type_id),' ',pd.objective) objective,
                ...
                WHERE rfq.order_type = 'quotation' AND rfq.order_id = selected_po.id) reason,
                pd.create_date::date
                ...
                ON selected_po.requisition_id = pd.id AND selected_po.order_type LIKE 'purchase_order' AND selected_po.state NOT LIKE 'cancel'
                WHERE pd.state = 'done'
