...
class PabiYearlyPurchaseReport(models.Model):
    ...
    def init(self, cr):
        ...
        cr.execute("""CREATE or REPLACE VIEW %s as (
                ...
                rc.name currency,
                (SELECT af.id FROM account_fiscalyear af WHERE af.name = to_char(pd.create_date,'YYYY')) fiscalyear_id
                ...
                (SELECT af.id FROM account_fiscalyear af WHERE af.name = to_char(pd.create_date,'YYYY'))
                ORDER BY ou.id
        )""" % (self._table, ))
