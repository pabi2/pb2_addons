...
class account_config_settings(osv.osv_memory):
    ...
    def set_default_loan_penalty_product_id(self, cr, uid, ids, context=None):
        wizard = self.browse(cr, uid, ids)[0]
        property_obj = self.pool.get('ir.property')
        field_obj = self.pool.get('ir.model.fields')
        todo_list = [
            ('property_loan_penalty_product_id',
    ...
    def get_default_loan_penalty_product_id(self, cr, uid,
                                            fields, context=None):
        ...
        res = {}
        for record in todo_list:
            prop = ir_property_obj.get(cr, uid, record[0],
                                       record[1], context=context)
