...
class sale_create_invoice_plan(models.TransientModel):
    ...
    def _default_order_amount(self):
        ...
        base_amount = order.price_include and order.amount_total or order.amount_untaxed
        return base_amount

    use_deposit = fields.Boolean('Advance on 1st Invoice', default=False)
    num_installment = fields.Integer('Number of Installment', default=0)
    installment_ids = fields.One2many('sale.create.invoice.plan.installment', 'plan_id', string='Installments')
    order_amount = fields.Float(string='Order Amount Untaxed', readonly=True, default=_default_order_amount)
    invoice_mode = fields.Selection([('change_price', 'As 1 Job (change price)'),
                                     ('change_quantity', 'As Units (change quantity)')], string='Invoice Mode', required=True, default='change_price')
    ...
    def _validate_total_amount(self):
        ...
        amount_total = sum([x.installment > 0 and x.amount or 0.0 for x in self.installment_ids])
        if round(amount_total, prec) != round(self.order_amount, prec):
            raise except_orm(_('Amount Mismatch!'),
    ...
    def do_create_invoice_plan(self):
        ...
        self.env['sale.invoice.plan']._validate_installment_date(self.installment_ids)
        ...
        if self.use_deposit:
            ...
            lines.append({
                ...
                'deposit_amount': round(self.installment_ids[0].percent/100 * base_amount, prec)
            })

        for install in self.installment_ids:
            if install.installment > 0:
                for order_line in order.order_line:
                    subtotal = order.price_include and (order_line.product_uom_qty * order_line.price_unit) or order_line.price_subtotal
                    lines.append({
                        'order_id': order.id,
...
class sale_create_invoice_plan_installment(models.TransientModel):
    ...
    plan_id = fields.Many2one('sale.create.invoice.plan', string='Wizard Reference', readonly=True)  
    ...
    percent = fields.Float(string='%', digits=(16,10))
