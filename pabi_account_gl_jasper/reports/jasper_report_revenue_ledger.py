...
class JasperReportRevenueLedger(models.TransientModel):
    _name = 'jasper.report.revenue.ledger'
    ...
    def _compute_org(self):
        ...
        if self.operating_unit_ids:
            domain_acc += [('company_id', '=', self.company_id.ids), ('user_type', '=', self.user_type.ids), ('operating_unit_id', 'in', self.operating_unit_ids.ids)]
            domain_partner += [('company_id', '=', self.company_id.ids), ('customer', '=', True), ('user_id.default_operating_unit_id', 'in', self.operating_unit_ids.ids)]
